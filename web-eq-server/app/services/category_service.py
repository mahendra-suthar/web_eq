...
class CategoryService:
    ...
    def delete_category(self, category_uuid: UUID) -> None:
        ...
        if has_children:
            raise HTTPException(
                status_code=409,
                detail="Cannot delete category with subcategories. Delete or reassign them first.",
            )
        has_services = self.db.query(Service).filter(Service.category_id == category_uuid).first()
        if has_services:
            raise HTTPException(
                status_code=409,
                detail="Cannot delete category that has services. Remove or reassign them first.",
            )
